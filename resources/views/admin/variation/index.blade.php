                        ...
                        <div class="box-body">
                          ...
                          <table class="table table-bordered">
                              ...
                              @foreach($table as $i)
                                  <tr>
                                      <td>{{$i->name}}</td>
                                      <td>@foreach($i->variationValues as $v) {{$v['value']}}, @endforeach</td>
                                      <td>{{$i->created_at}}</td>
                                      <td>
                                          <form action="{{route('variation.show',$i->id)}}" method="GET">
                                          <button type="submit" class="btn btn-xs btn-info"><i class="fa fa-edit text-light"></i></button>
                                          </form>
                                      </td>
                                      <td>
                                          <form action="{{route('variation.destroy',$i->id)}}" method="POST">
                                              @method('Delete')
                                              @csrf
